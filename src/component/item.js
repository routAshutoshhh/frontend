...
import { fetchItems, addItem, updateItem, deleteItem } from "../apiService";
  ...
  return (
    <div>
      ...
      <ul>
        {items.map((item) => (
          <li key={item.item_id}>
            {item.item_name} - {item.quantity} - {item.cost_price} -{" "}
            {item.sale_price} - {item.description} - {item.image_url}
            <button
              onClick={() =>
